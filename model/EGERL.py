...
class EGERL:
    ...
    def train_and_eval(self):
        ...
        print(len(d.relations))
        self.entity_idxs = {d.entities[i]: i for i in range(len(d.entities))}
        self.relation_idxs = {d.relations[i]: i for i in range(len(d.relations))}
        print(len(self.entity_idxs))
        print(len(self.relation_idxs))
        train_data_idxs = self.get_data_idxs(d.train_data)
        print("Number of training data points: %d" % len(train_data_idxs))

        print("Creating the new weighted Adj matrix!")
        adj = compute_weighted_adj_matrix(d.train_data, self.entity_idxs, self.relation_idxs).to(device)  #important
        ...
        #lst_indexes = torch.LongTensor([i for i in range(len(d.entities) + len(d.relations)-18)]).to(device)
        ...
        print("训练时长为：",  time_diff)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset", type=str, default="data-全局节点/data-shanchu/train-全局-min_anchorlink_全局节点.txt", nargs="?", help="codex-s, codex-m, and codex-l")
    ...
    print("开始训练时间为：", start_time)
    gnnkge = EGERL(encoder=args.encoder, decoder=args.decoder, num_iterations=args.num_iterations, batch_size=args.batch_size,
